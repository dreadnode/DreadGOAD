...
def test_environment_provider_override_matches_cli_resolution() -> None:
    ...
    try:
        ...
        assert listing["env_regions"]["service-dev"] == "centralus", listing

        snap = derive_snapshot(tmp.name, "service-dev")
        assert snap["provider"] == "azure", snap
        assert snap["region"] == "centralus", snap
        assert snap["lab"] == "ad/SERVICE", snap
...
def test_session_lab_config_path_falls_back_and_refuses_env_escape() -> None:
    ...
    session = t.cast(
        SessionDocument,
        {
            ...
            "snapshot": {"lab": "ad/SERVICE"},
        },
    )
    expected_base = _REPO / "ad" / "SERVICE" / "data" / "config.json"
