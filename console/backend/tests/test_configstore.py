...
def test_managed_path_is_stable_bounded_and_contained() -> None:
    with isolated_state():
        root = paths.configs_root().resolve()
        first = configstore.managed_path_for("SERVICE", "azure", "kraken")
        assert first == configstore.managed_path_for("SERVICE", "azure", "kraken")
        assert first.parent == root
        assert first.name.startswith("service-azure-kraken-")
