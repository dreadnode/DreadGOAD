...
const options: SessionOptions = {
  ...
  environments: [{
    name: 'service-dev', lab: 'SERVICE', provider: 'azure',
    deployment: 'service-deployment', region: 'centralus', variant: false,
  ...
  ranges: [
    ...
    {
      name: 'SERVICE', display_name: 'SERVICE', dir: 'ad/SERVICE',
      kind: 'service-range', providers: ['azure'], hosts: ['web01'],
      variant_supported: false, generated: false,
      provider_settings: {
        azure: {
          deployment: 'service-deployment', scaffold_profile: 'template',
          template_environment: 'service-dev', default_region: 'centralus',
...
const service = deriveNewSessionModel({
  ...base, rangeName: 'SERVICE', environmentName: 'kraken',
})
assert.equal(service.valid, true)
assert.equal(service.effectiveProvider, 'azure')
assert.equal(service.variantAvailable, false)
assert.equal(service.cidrEditable, false)
assert.deepEqual(service.payload, {
  mode: 'create_range', range: 'SERVICE', provider: 'azure',
...
assert.deepEqual(existing.payload, {
  config_path: '/repo/dreadgoad.yaml', env: 'service-dev',
})
