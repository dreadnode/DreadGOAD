...
const serviceHost = { ...host('services01', 'member'), source: 'config', os: 'linux' }
const serviceMarkup = renderToStaticMarkup(
  createElement(HostNode, { data: serviceHost } as unknown as Parameters<typeof HostNode>[0]),
)
assert.ok(serviceMarkup.includes('>Linux</span>'))
assert.ok(!serviceMarkup.includes('Windows Server'))
