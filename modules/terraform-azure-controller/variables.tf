...
variable "plan" {
  description = "Marketplace plan terms for the source image. Required when booting from a gallery image whose underlying source carries plan terms (e.g. Kali). The plan must also be accepted in the subscription via `az vm image terms accept --publisher <p> --offer <o> --plan <n>`. Leave null for plan-free images (Ubuntu marketplace, custom managed images)."
  type = object({
    name      = string
    product   = string
    publisher = string
