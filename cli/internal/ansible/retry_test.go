...
func TestRetryLimit(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "ordinary retry targets failed hosts",
			opts:        RetryOptions{},
			failedHosts: []string{"dev01"},
			want:        "dev01",
		},
		{
			name:        "all-host retry does not narrow an unrestricted run",
			opts:        RetryOptions{RetryAllHosts: true},
			failedHosts: []string{"dev01"},
			want:        "",
		},
		{
			name: "all-host retry preserves an explicit user limit",
			opts: RetryOptions{
				Limit:         "dev01,storage01",
				RetryAllHosts: true,
			},
			failedHosts: []string{"dev01"},
			want:        "dev01,storage01",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := retryLimit(tt.opts, tt.failedHosts); got != tt.want {
				t.Errorf("retryLimit() = %q, want %q", got, tt.want)
			}
		})
	}
}
