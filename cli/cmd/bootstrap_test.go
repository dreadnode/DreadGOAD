...
func TestEnsureInventorySyncedSkipsNonAWSProvider(t *testing.T) {
	dir := t.TempDir()
	invPath := filepath.Join(dir, "test-inventory")

	// Write a Ludus-style inventory. Provider configuration, rather than a
	// possibly stale ansible_connection value, decides that AWS sync is skipped.
