...
def cmd_generate_key(args):
    from .answer_key.generator import generate_answer_key
    import json
    ...
    output_path = args.output or str(
        Path(__file__).parent / "answer_key" / "answer_key.json"
    )
...
def cmd_run(args):
    from .verifier.verify import load_answer_key
    from .tui.app import run_tui

    # Load answer key
    key_path = args.answer_key or str(
        Path(__file__).parent / "answer_key" / "answer_key.json"
    )
    ...
    if args.transport == "local":
        ...
        if not args.instance_id:
            ...
            sys.exit(1)
        from .transport.ssm import SSMTransport
...
def cmd_demo(args):
    import random
    from .answer_key.generator import generate_answer_key
    from .verifier.verify import verify_report
    from .tui.app import build_status_board
