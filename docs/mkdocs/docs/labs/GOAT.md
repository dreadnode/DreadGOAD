...
**GOAT** (**Game of Agent Trust**) is a six-host Linux environment for exercising offensive AI agents
...
The final `goat-seed.yml` play adds a versioned, repeatable activity layer
after the foundational services are ready. `goat-seed-v2` creates four Garage
...
`goat-seed-v2` changes persistent database and application principals as well
as the scenario data. A range previously provisioned with `goat-seed-v1` must
...
./cli/dreadgoad --env goat-dev config show
./cli/dreadgoad --env goat-dev infra validate
./cli/dreadgoad --env goat-dev infra plan
./cli/dreadgoad --env goat-dev infra apply
./cli/dreadgoad --env goat-dev provision
```
...
`us-east-2`). The CLI scaffolds from the `goat-aws` template and automatically
...
`~/.dreadgoad/keys/azure-goat-dev-goat-admin`. Azure resources use the
`<environment>-goat-*` prefix, including the `goat-dev-goat-rg` resource group.
...
These command names are range-agnostic at the console surface. The range
manifest selects the GOAT-specific operations, inspection, and validation
implementations; command handlers do not branch on the range name. GOAT
declares no session initialization hooks,
...
`goat-kali.yml`, the final foundational provisioning play, waits for the core
endpoints and runs `/usr/local/bin/goat-smoke` from Kali. The smoke test
...
`goat-seed.yml` play reconciles and verifies the versioned activity layer.

To rerun the live service gate without rebuilding infrastructure:

```bash
./cli/dreadgoad --env goat-dev provision --plays goat-kali.yml
```

To reconcile only the versioned activity layer:

```bash
./cli/dreadgoad --env goat-dev provision --plays goat-seed.yml
```
...
`ad/GOAT/data/validation.json`. On Azure it checks the resource group,
...
./cli/dreadgoad --env goat-dev validate --quick

# Select a subscription and report path explicitly
AZURE_SUBSCRIPTION_ID="<subscription-id>" \
./cli/dreadgoad --env goat-dev validate \
  --output /tmp/goat-validation.json

# Validate both provider contracts without contacting either cloud
./scripts/validate-goat-live.py --provider azure --manifest-only
./scripts/validate-goat-live.py --provider aws --manifest-only
```
...
The CLI dispatches `GOAT` to `scripts/validate-goat-live.py` and
...
```bash
./scripts/validate-goat.sh
```

## Access and synthetic credentials

Connection settings are installed on Kali at
`/home/kali/.config/goat/services.env` with mode `0600`. The file includes
...
`/opt/goat-agent` and use `/home/kali/workspace` for agent work.
...
```bash
./cli/dreadgoad --env goat-dev infra destroy
```

Terraform state is stored beneath `~/.dreadgoad/state/azure/goat/`,
...
by early GOAT builds. Back up this directory while a range is live: it is
