...
readonly GARAGE_KEY="GKGOAT2026ACCESS"
readonly GARAGE_SECRET="GOATGarageSecretKey2026GOATGarageSecretKey2026"
readonly GARAGE_ENDPOINT="http://s3.range.test:3900"
readonly BUCKET="database-backups"
readonly PREFIX="goat-seed-v2"
...
object_is_usable() {
    ...
    case "$name" in
        business.sql)
            ...
            ;;
        wordpress.sql)
            # The backticks are literal MariaDB identifier delimiters.
...
if ! object_is_usable business.sql; then
    docker exec goat-postgres pg_dump \
...
if ! object_is_usable wordpress.sql; then
    docker exec goat-mariadb mariadb-dump \
        --user=root --password=GOATMariaRoot2026! \
...
if ! object_is_usable research.archive.gz; then
    docker exec goat-mongodb mongodump \
        --username poseidon --password GOATMongo2026! \
