...
set -a
source /home/kali/.config/goat/services.env
set +a
...
curl --fail --silent --show-error \
    --user "michael:$FILES_PASSWORD" "$GITEA_URL/api/v1/user" \
    | jq --exit-status '.login == "michael"' > /dev/null
curl --fail --silent --show-error "$REGISTRY_URL/v2/" > /dev/null
PGPASSWORD="$POSTGRES_PASSWORD" psql \
    --host "$POSTGRES_HOST" --username "$POSTGRES_USER" --dbname business \
    --tuples-only --command 'SELECT count(*) FROM customers' | grep -Eq '[[:space:]]*12'
mysql \
    --host "$MARIADB_HOST" --user "$MARIADB_USER" \
    --password="$MARIADB_PASSWORD" wordpress \
    --batch --skip-column-names --execute 'SELECT count(*) FROM range_notes' | grep -qx '6'
redis-cli -u "$REDIS_URL" GET session:michael | grep -qx active
ldapsearch -LLL -x -H "$LDAP_URL" \
    -D "$LDAP_BIND_DN" -w "$LDAP_BIND_PASSWORD" \
    -b ou=people,dc=range,dc=test '(uid=michael)' uid | grep -qx 'uid: michael'
