...
ADMIN_AUTH = ("poseidon", "GOATGitea2026!")
LDAP_AUTH = {
    "michael": ("michael", "GOATFiles2026!"),
    "shane": ("shane", "GOATFiles2026!"),
}
SOURCE = pathlib.Path("/opt/goat/development/seed/kraken-control-plane")
SEED_TAG = "v0.1.0"
SEED_VERSION = "goat-seed-v2"
...
def ensure_repository_history() -> str:
    ...
    with tempfile.TemporaryDirectory(prefix="goat-gitea-seed-") as directory:
...
def ensure_actions_configuration() -> None:
    ...
    for name, value in {
        "REGISTRY_PASSWORD": "GOATRegistry2026!",
        "GARAGE_SECRET_KEY": "GOATGarageSecretKey2026GOATGarageSecretKey2026",
    }.items():
        api(
...
def upload_release_asset(release: dict[str, Any], content: bytes) -> None:
    ...
    if any(asset.get("name") == name for asset in release.get("assets", [])):
        return
    boundary = f"goat-{uuid.uuid4().hex}"
    payload = (
        (
            f"--{boundary}\r\n"
    ...
    if release is None:
        release = api(
            ...
            body={
                ...
                "name": "KRAKEN 0.1.0",
                "body": "Synthetic release generated by goat-seed-v2.",
                "draft": False,
                "prerelease": False,
            },
            allowed=(201,),
