#!/usr/bin/env python3
"""Validate a deployed Azure or AWS GOAT against its expected-state manifest."""
...
DEFAULT_MANIFEST = SCRIPT_DIR.parent / "ad" / "GOAT" / "data" / "validation.json"
DEFAULT_AZURE_INFRA_ROOT = (
    SCRIPT_DIR.parent / "infra" / "azure" / "goat-deployment"
)
DEFAULT_AWS_INFRA_ROOT = SCRIPT_DIR.parent / "infra" / "goat-deployment"
...
REMOTE_RESULT_PREFIX = "GOAT_RESULT "
...
spec="$(printf '%s' "$GOAT_VALIDATION_SPEC_B64" | base64 -d)" || exit 2
...
def validate_name_template(
    ...
    try:
        rendered = template.format(
            env="goat-validation", deployment="goat", host="web01"
        )
    except (AttributeError, IndexError, KeyError, ValueError) as exc:
        raise ValueError(f"{label} is invalid: {template!r}") from exc
    expected_values = {"env": "goat-validation", "deployment": "goat", "host": "web01"}
...
def load_manifest(path: pathlib.Path, provider: str = "azure") -> dict[str, Any]:
    """Load and structurally validate the GOAT manifest."""
    try:
    ...
    if manifest.get("lab") != "GOAT" or provider not in manifest.get(
        "providers", []
    ):
        raise ValueError(f"manifest must describe the {provider} GOAT lab")
...
def build_remote_launcher(
    ...
    payload = f"GOAT_VALIDATION_SPEC_B64={shlex.quote(encoded_spec)}\n{REMOTE_RUNNER}"
...
def main(argv: Sequence[str] | None = None) -> int:
    """Run GOAT validation and return a process exit status."""
    ...
    if args.manifest_only:
        print(f"GOAT validation manifest is valid: {args.manifest}")
        return 0
    ...
    if output is None and not args.json:
        output = pathlib.Path(f"/tmp/goat-validation-{timestamp}.json")
    if not args.json:
        print("==========================================")
        print("GOAT Live Validation")
        print("==========================================")
