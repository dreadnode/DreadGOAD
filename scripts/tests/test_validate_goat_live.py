"""Unit tests for the standalone GOAT live validator."""
...
VALIDATOR_PATH = REPO_ROOT / "scripts" / "validate-goat-live.py"
SPEC = importlib.util.spec_from_file_location("goat_validator", VALIDATOR_PATH)
...
class RemoteExecutionTests(unittest.TestCase):
    ...
    def test_aws_remote_checks_use_linux_ssm_document(self) -> None:
        line = (
            'GOAT_RESULT {"status":"PASS","category":"Data",'
    ...
    def test_remote_parser_ignores_wrapper_and_invalid_lines(self) -> None:
        message = "\n".join(
            [
                ...
                'GOAT_RESULT {"status":"PASS","category":"Data","name":"database","detail":"ok"}',
                "GOAT_RESULT not-json",
                'GOAT_RESULT {"status":"UNKNOWN","category":"Data","name":"ignored"}',
                "[stderr]",
            ]
        )
...
class InfrastructureTests(unittest.TestCase):
    ...
    def test_aws_duplicate_name_tag_fails_exact_instance_set(self) -> None:
        ...
        self.assertIn("goat-aws-goat-web01': 2", exact_check.detail)
