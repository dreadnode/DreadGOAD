...
def _range() -> dict:
    def host(hid, role, source="config"):
        return {"id": hid, "hostname": hid, "role": role, "source": source,
                "status": "unknown", "health": "unknown", "ip_private": None,
                "ip_public": None, "cloud_id": None, "last_checked_at": None}
    return {
        "session_id": "s-1",
        "hosts": [
        ...
        "edges": [], "layout": {}, "last_checked_at": None,
    }


def test_matched_host_gets_state_ip_id() -> None:
    instances = [
        {"name": "goad-dreadgoad-kingslanding-vm", "id": "i-0abc",
         "state": "running", "private_ip": "10.0.4.124"},
    ]
...
def test_unmatched_config_host_is_absent() -> None:
    instances = [{"name": "x-kingslanding-vm", "id": "i-1", "state": "running", "private_ip": "1.2.3.4"}]
    out = map_range_status(_range(), instances, now="T")
    hosts = {h["id"]: h for h in out["hosts"]}
...
def test_unmatched_instance_ignored_no_new_node() -> None:
    instances = [{"name": "some-random-elk-vm", "id": "i-9", "state": "running", "private_ip": "9.9.9.9"}]
    out = map_range_status(_range(), instances, now="T")
...
def test_infra_alias_matches_kali() -> None:
    instances = [{"name": "goad-dreadgoad-kali", "id": "i-kali", "state": "running", "private_ip": "10.0.4.9"}]
    out = map_range_status(_range(), instances, now="T")
    hosts = {h["id"]: h for h in out["hosts"]}
    assert hosts["attackbox"]["status"] == "running", "attackbox should match the kali VM"
...
def test_state_normalization() -> None:
    for cloud, expected in [("running", "running"), ("stopped", "stopped"),
                            ("deallocated", "stopped"), ("pending", "provisioning"),
                            ("terminated", "absent"), ("weird", "unknown")]:
        instances = [{"name": "x-kingslanding-y", "id": "i", "state": cloud, "private_ip": ""}]
        out = map_range_status(_range(), instances, now="T")
...
def test_summarize_changes() -> None:
    before = _range()
    after = map_range_status(before, [
        {"name": "x-kingslanding-y", "id": "i", "state": "running", "private_ip": ""},
    ], now="T")
